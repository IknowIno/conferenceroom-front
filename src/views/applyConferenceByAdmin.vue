<template>
    <div>
        <div class="warnning">
            <el-alert title="提示" type="error" description="请尽量在会议开始前1小时之前进行申请，且每个会议之间间隔30分钟以供特殊情况时使用"
                style="border-radius: 30px" :closable="false">
            </el-alert>
        </div>
        ...
        <div class="mConditions">
            ...
            <el-select class="mselect" v-model="roomType" clearable placeholder="类型" @change="change">
                <el-option v-for="item in types" :key="item.roomType" :label="item.roomType" :value="item.roomType">
                </el-option>
            </el-select>

            <el-select class="mselect" v-model="roomSize" clearable placeholder="可容纳人数" @change="change">
                <el-option v-for="item in sizes" :key="item.roomSize" :label="item.roomSize" :value="item.roomSize">
                </el-option>
            </el-select>

        </div>
        <el-table
            :data="conferenceRooms.filter(data => !this.search || data.roomNo.toLowerCase().includes(this.search.toLowerCase()) ||
                    data.roomFloor.toLowerCase().includes(this.search.toLowerCase()) || data.roomType.toLowerCase().includes(this.search.toLowerCase()))"
            style="width: 100%;text-align: center" :row-class-name="this.tableRowClassName">
            <el-table-column prop="roomNo" label="门牌号" width="150">
            </el-table-column>
            <el-table-column prop="roomFloor" label="楼层" width="150">
            </el-table-column>
            <el-table-column prop="roomType" label="类型" width="250">
            </el-table-column>
            <el-table-column prop="roomSize" label="可容纳人数" width="180">
            </el-table-column>
            ...
            <el-table-column align="right">
                <template slot="header" slot-scope="scope">
                    <el-input v-model="search" size="mid" placeholder="输入楼层、会议室门牌号、会议室类型" />
                </template>
                <template slot-scope="scope">
                    <el-button size="medium" @click="handleApply(scope.$index, scope.row)" round type="warning"
                        style="margin-right: 10px">申 请</el-button>
        ...
        <el-dialog title="申请会议室" :visible.sync="dialogFormApply">
            <el-form :model="record.conferenceRecord" label-position="right" label-width="80px" status-icon
                :rules="rules" ref="applyForm">

                <el-form-item label="申请部门" prop="depId">
                    <el-select v-model="record.conferenceRecord.depId" clearable placeholder="请选择部门"
                        style="width: 250px">
                        <el-option v-for="item in deps" :key="item.depId" :label="item.depName" :value="item.depId">
                        </el-option>
                    </el-select>
                </el-form-item>

                <el-form-item label="会议主题" prop="theme">
                    <el-input v-model="record.conferenceRecord.theme" placeholder="请输入会议主题"
                        style="width: 250px"></el-input>
                </el-form-item>
                <el-form-item label="会议介绍" prop="digest">
                    <el-input type="textarea" autosize v-model="record.conferenceRecord.digest" placeholder="请输入会议摘要"
                        style="width: 250px">
                    </el-input>
                </el-form-item>
                <el-form-item label="会议人数" prop="personCount">
                    <el-input @blur="examinePersonCount" v-model.number="record.conferenceRecord.personCount"
                        placeholder="会议人数" style="width: 250px">
                    </el-input>
                </el-form-item>

                <el-form-item label="开始时间" prop="startTime">
                    <el-date-picker style="width: 250px" v-model="record.conferenceRecord.startTime" type="datetime"
                        placeholder="选择开始日期时间" :picker-options="pickerOptions" @blur="searchTimeConflict"
                        default-time="10:00:00" value-format="yyyy-MM-dd HH:mm:ss">
                    </el-date-picker>
                </el-form-item>

                <el-form-item label="结束时间" prop="endTime">
                    <el-date-picker style="width: 250px" v-model="record.conferenceRecord.endTime" type="datetime"
                        placeholder="选择结束日期时间" :picker-options="pickerOptions" @blur="searchTimeConflict"
                        default-time="10:30:00" value-format="yyyy-MM-dd HH:mm:ss">
                    </el-date-picker>
                </el-form-item>
        ...
        <el-dialog title="设备" :visible.sync="dialogTableVisible" style="text-align: center">
            <el-table :data="devices" style="text-align: center" :row-class-name="tableRowClassName">
                <el-table-column prop="dname" label="设备名称">
                </el-table-column>
                <el-table-column prop="dnumber" label="数量">
